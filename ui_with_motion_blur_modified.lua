...
RS.RenderStepped:Connect(function(dt)
    if targetPos then
        local current = Window.Position
        Window.Position = UDim2.new(
            0, current.X.Offset + (targetPos.X.Offset - current.X.Offset) * 0.25,
            0, current.Y.Offset + (targetPos.Y.Offset - current.Y.Offset) * 0.25
        )
    end
end)
...
-- Motion blur-like effect (clean motion moving, no blur)
local TS = game:GetService("TweenService")
local function applyMotionEffect(gui)
    gui.Changed:Connect(function(prop)
        if prop == "Position" then
            local tween = TS:Create(gui, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = gui.Position})
            tween:Play()
        end
    end)
end

applyMotionEffect(Window)
